...
public final class S2Loop implements S2Region, Comparable<S2Loop>, Serializable, S2Shape {
  ...
  public boolean contains(S2Cell target) {
    S2Iterator<Cell> it = index.iterator();
   ...
   * returns INDEXED.)
   */
  private boolean boundaryApproxIntersects(S2Iterator<Cell> it, S2Cell target) {
  ...
  /**
   * Given an iterator that is already positioned at the {@link Cell} containing {@code
   * p}, returns contains(p).
   */
  private boolean contains(S2Iterator<Cell> it, S2Point p) {
  ...
  private static final class LoopCrosser {
    ...
    private final List<Cell> bCells;
